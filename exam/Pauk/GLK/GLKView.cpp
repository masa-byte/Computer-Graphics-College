...
{
	if ( nChar == VK_LEFT )
		m_glRenderer.yRot -= 10.0f;
	else if ( nChar == VK_RIGHT )
		m_glRenderer.yRot += 10.0f;
	else if (nChar == VK_UP)
	{
		m_glRenderer.xRot == 179 ? m_glRenderer.xRot = m_glRenderer.xRot : m_glRenderer.xRot += 2.0f;
	}
	else if (nChar == VK_DOWN)
	{
		m_glRenderer.xRot == 2 ? m_glRenderer.xRot = m_glRenderer.xRot : m_glRenderer.xRot -= 2.0f;
	}
	else if (nChar == 'Q')
	{
		m_glRenderer.zTrans == -8 ? m_glRenderer.zTrans = m_glRenderer.zTrans : m_glRenderer.zTrans += 2.0f;
	}
	else if (nChar == 'W')
	{
		m_glRenderer.zTrans == -50 ? m_glRenderer.zTrans = m_glRenderer.zTrans : m_glRenderer.zTrans -= 2.0f;
	}
	Invalidate();
